...
library(dplyr)
myco_in <- read.csv("https://mycocosm.jgi.doe.gov/ext-api/mycocosm/catalog/download-group?flt=&seq=all&pub=all&grp=fungi&srt=released&ord=desc", check.names = F, col.names = c("Name", "portal", "NCBI_TaxID", "assembly length", "gene_count", "is_restricted", "is_public", "is_published", "is_superseded","superseded by", "publications", "pubmed_id","doi_id")) %>% arrange(NCBI_TaxID)
